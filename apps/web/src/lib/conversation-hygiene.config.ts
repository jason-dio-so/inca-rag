/**
 * STEP 3.7-γ: Conversation Hygiene Configuration
 *
 ...
 * 3. EXACT 상태 우선 원칙 - EXACT일 때만 Chat 로그에 정상 응답 추가
 */
...
export interface CoverageGuideState {
  /** 현재 가이드 상태 (AMBIGUOUS / NOT_FOUND / null) */
  resolutionState: UIResolutionState | null;
  /** 표시할 메시지 */
  message: string;
  /** 선택 가능한 담보 목록 (AMBIGUOUS 상태에서만) */
...
export function shouldShowCoverageGuide(
  ...
  const state = getUIResolutionState(resolution);
  return state === "AMBIGUOUS" || state === "NOT_FOUND";
}
 ...
 * - EXACT: null 반환 (가이드 불필요)
 * - AMBIGUOUS / NOT_FOUND: 가이드 상태 생성
 */
